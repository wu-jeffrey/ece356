...
//             0.11: 27 September 2021: Implemented -l and -n logic
//
// Acknowledgements:
...
//
  ...
  const char* USERID   = "j387wu";                          // Your UserID here
  ...
  const char* selectionClause;
  const char* orderingClause = " order by schema_name;";
  const char* pattern = argv[2];
  
  if (argc == 3) {
  	if (!strncmp("-n", argv[1], 2)) {
		  selectionClause = " where schema_name not like '";
  	} else if (!strncmp("-l", argv[1], 2)) {
		  selectionClause = " where schema_name like '";
  	}
  }
  ...
  if (argc == 3) {
	  queryLen += (
					strlen(selectionClause) +
				 	strlen(pattern) +
				 	2  // Need 3 more chars for single quote, space and null terminator 
				);              
  }
  ...
  if (argc == 3) {
  	strcat(queryBuffer, selectionClause);
  	strcat(queryBuffer, pattern);
	  strcat(queryBuffer, "' "); // Close single quote for pattern
  }

  strcat(queryBuffer, orderingClause);

  // Handy debugging statement to ensure I have a good query
#ifdef DEBUG
  char debugBuffer[strlen(pattern)];
  strcpy(debugBuffer, pattern);
  
  cout << "Query is: " << queryBuffer << endl
       << "with length: " << strlen(queryBuffer) << " and queryLen = " << queryLen << endl
	   << "pattern: " << debugBuffer << endl;
#endif
